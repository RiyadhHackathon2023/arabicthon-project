...
from ...storage_manager import get_swift_connection, SwiftConnection
import uuid
...
async def get_sources_by_type(source_type: SourceTypeEnum):
    ...
    session = get_session()
    source_query = session.query(SourceModel)
    sources: List[SourceModel] = source_query.filter(SourceModel.source_type == source_type).all()
...
async def get_sources():
    ...
    session = get_session()
    source_query = session.query(SourceModel)
    sources: List[SourceModel] = source_query.all()
