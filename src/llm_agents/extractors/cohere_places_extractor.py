from src.llm_agents.extractors.examples.places_examples import places_examples
from src.llm_agents.extractors.cohere_extractor import cohereExtractor

class coherePlacesExtractor(cohereExtractor):
    def __init__(self):
        super().__init__()
        self.examples = [e[1] for e in places_examples]
        self.example_labels = [e[0] for e in places_examples]
        self.example_prompt = "Extract in Arabic all the names of historical places(or landmarks) from the text: "
    
    def extract(self, example):
        extracted = super().extract(example)
        return extracted.split(',')